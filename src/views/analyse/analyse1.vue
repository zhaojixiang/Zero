  ...
  <section id="analyse">
    ...
    <section class="container">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="16">
          <!-- 总览 -->
          <overview :ratioPieOption='ratioPieOption' :rateCover='rateCover'></overview>            
...
export default {
  ...
  methods: {
    ...
    energyTrace(val) {
      ...
      request.spectrumEvaluation.GetTrace(param).then(res=>{
    ...
    _getRateList() {
      this.$http.get("apiGetList").then(res => {
        ...
        res.data.data.forEach(item => {
          this.datas.push({
            rate: item.one,
            band_width: item.two,
            rate_cover: item.three,
            natrue: item.four
          });
        });
        // console.log('api get list :',this.datas);
      });
      // let param = {
      //   CID,
      //   StartTime:START_TIME,
      ...
      //   ThresholdType:THRES_HOLD_TYPE
      // }
      // request.spectrumEvaluation.GetSignal(param).then(res=>{
    ...
    _getFixedStationList() {
      this.datas = [];
      this.$http.get("api/GetFixedStationList").then(res => {
    ...
    //   }
    //   // console.info(param)
    //   request.spectrumEvaluation.GetChannel(param).then(res=>{
    //     // console.log('channel: ',res)
    //     let { Data,Msg,Success } = res
    //     if(Success) {
    //       let arr = Object.entries(Data.Freq_ChannelOccupys)
    //       // arr.forEach(item=>{
    //       //   let [rate,band_width] = item
    //       //   this.datas.push({
    //       //     rate:rate,
    ...
    _getTrace() {
      ...
      request.spectrumEvaluation
        .GetTrace(param)
        .then(res => {
          console.log("============ trace", res);
          let { Data, Msg, Success } = res;
          if (Success) {
            let { MaxIndex, MaxPower, MinPower, Positions, Powers } = Data;
          } else {
            this.$message({
              type: "warning",
              message: Msg ? Msg : "暂时没有数据"
