<template>
  <section>
    ...
    <div class="main">
        ...
        <ul class="legend-ul">
          <li v-for="item in legend">
            <section class="legend-li" v-if="item.span != undefined || item.icon != undefined">
              <span class="color-block" style="border: solid 1px #fff;width: 20px;height: 15px;text-align: center"
                    v-if="item.icon!=undefined">
                <i :class="'fa fa-1 '+item.icon"></i>
              </span>
              <span class="color-block" style="border: solid 1px #fff;width: 20px;height: 15px;text-align: center"
                    v-else>{{ item.span }}</span>
      ...
      <div class="content">
        <div class="box-first">
          <v-chart :force-fit="true" :height="250" :data="data" :data-view="dataView" :data-pre="dataPre" :scale="false"
                   :padding="['18%', '1%', '6%', '1%']">
            <v-tooltip :show-title="false" :item-tpl="itemTpl"/>
            <v-polygon :onClick="click" :position="'x*y'" :color="color" :tooltip="tooltip" :v-style="style"
                       :label="label"/>
            <v-guide v-for="(n,index) in nodes" :key="index" type="text" :zIndex="1000" :offsetY="0" :top="true"
                     :vStyle="guideStyle" :position="position(n)" :content="n.data.freq"/>
            <v-guide type="text" :zIndex="1000" :offsetY="0" :top="true" :vStyle="guideStyle" :position="[1,1]"
                     :content="300"/>
            <v-guide type="text" :zIndex="1000" :offsetY="10" :top="true" :vStyle="{fill: '#fff',fontSize: '12'}"
                     :position="[0,0]" :content="'30MHz'"/>
            <v-guide type="text" :zIndex="1000" :offsetY="10" :offsetX="-60" :top="true"
                     :vStyle="{fill: '#fff',fontSize: '12'}" :position="[1,0]" :content="'300MHz'"/>
          </v-chart>
        </div>
        <div class="box-second">
          <v-chart :force-fit="true" :height="300" :data="data2" :data-view="dataView" :data-pre="dataPre"
                   :scale="false" :padding="['10%', '1%', '6%', '1%']">
            <v-tooltip :show-title="false" :item-tpl="itemTpl"/>
            <v-polygon :onClick="click" :position="'x*y'" :color="color" :tooltip="tooltip" :v-style="style"
                       :label="label"/>
            <v-guide v-for="(m,index) in nodes2" :key="index" type="text" :zIndex="1000" :offsetY="0" :top="true"
                     :vStyle="guideStyle" :position="position(m)" :content="m.data.freq"/>
            <v-guide type="text" :zIndex="1000" :offsetY="0" :top="true" :vStyle="guideStyle" :position="[1,1]"
                     :content="3000"/>
            <v-guide type="text" :zIndex="1000" :offsetY="10" :top="true" :vStyle="{fill: '#fff',fontSize: '12'}"
                     :position="[0,0]" :content="'300MHz'"/>
            <v-guide type="text" :zIndex="1000" :offsetY="10" :offsetX="-60" :top="true"
                     :vStyle="{fill: '#fff',fontSize: '12'}" :position="[1,0]" :content="'3GHz'"/>
          </v-chart>
        </div>
      </div>
    </div>
...
<script>
  ...
  import {
    ...
    label,
    guideStyle
  } from '../../assets/js/chartOptions/frequencyAllocationOption'
  ...
  //todo 将生成的结构直接替换为data静态数据
  for (let i = 0; i < data.children.length; i++) {
    if (i != (data.children.length - 1)) {
      data.children[i].endFreq = data.children[i + 1].freq;
    } else {
      data.children[i].endFreq = 300;
    }
    let freq = data.children[i].freq;
    let endFreq = data.children[i].endFreq;
    if (!data.children[i].children) {
      continue
    }
    for (let j = 0; j < data.children[i].children.length; j++) {
      data.children[i].children[j].freq = freq;
      data.children[i].children[j].endFreq = endFreq;
    }
  }
  const data2 = {
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          {
            value: 20,
            legend: LEGEND.RADIO_NAVIGATION,
            span: LEGEND.SATELLITE.span,
            label: LEGEND.SKY_TO_SKY.icon,
            label2: LEGEND.SKY_TO_LAND.icon
          },
          {value: 20, legend: LEGEND.RADIO_LOCATION},
        ],
      },
      ...
      },
      {
        ...
        children: [
          ...
          {
            value: 20,
            legend: LEGEND.RADIO_DETERMINATION_SATELLITE,
      ...
      },
      {
        ...
        children: [
          ...
          {
            value: 30,
            legend: LEGEND.EARTH_EXPLORATION_SATELLITE,
            label: LEGEND.SKY_TO_SKY.icon,
            label2: LEGEND.SKY_TO_LAND.icon
          },
  ...
  for (let i = 0; i < data2.children.length; i++) {
    if (i != (data2.children.length - 1)) {
      data2.children[i].endFreq = data2.children[i + 1].freq;
    } else {
      data2.children[i].endFreq = 3000;
    }
    let freq = data2.children[i].freq;
    let endFreq = data2.children[i].endFreq;
    if (!data2.children[i].children) {
      continue
    }
    for (let j = 0; j < data2.children[i].children.length; j++) {
      data2.children[i].children[j].freq = freq;
      data2.children[i].children[j].endFreq = endFreq;
    }
  }
  ...
  const nodes2 = dv2.getAllNodes();
  nodes.map(node => {
    node.freq = node.data.freq;
  ...
  export default {
    ...
    mounted() {
      if(this.$store.state.cityData[0]) {
        let city = this.$store.state.cityData[0];
    ...
    methods: {
      click: function (e) {
        this.$router.push({
          ...
          query:{
            startFreq:e.data._origin.startFreq,
            stopFreq:e.data._origin.endFreq,
          },
        });
        // console.log(e);
      }
