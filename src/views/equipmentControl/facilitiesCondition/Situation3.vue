  ...
  <div class="content" :hide="currentLocation">
    ...
    <div class="r-box" v-if="!mapShow">
      <div class="center">
        ...
        <div class="mapbg">监测设施总数：<span>{{equipCount}}个</span>，设备制造商总数 : {{ integraCount }}家</div>
    ...
    <div id="map2" class="r-box" v-if="mapShow">
      <div class="areabg"><p>监测设施总数：<span>{{cityEquipCount}}个</span>，设备制造商总数 : {{ integraCount }}家</p></div>
      <map2 ref="map"/>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      initPie:function(){
        this.$http.get('api/EquipSurvey/ManuCountStatistics',{
          ...
          this.integraOption = GetIntegraOption(dateData.endyear+'年全'+this.locationLevelStr+'各设备制造商设施数量统计',legend,res.data);
        });
      },
      initMap:function(){
      ...
      initMultiBar(){
        this.$http.get('api/EquipSurvey/LastYearsInteInfo',{
          ...
          this.multiBarOption = GetMultiBarOption('最近10年全'+this.locationLevelStr+'各设备制造商设施数量统计', dateData.xData, list);
        });
      },
      initCity(){
        this.$http.get('api/EquipSurvey/AllCitierManuStatistics',{
            procode:this.$store.state.procode,
            citycode:this.cityCode,
            year:dateData.endyear
          }).then(res => {
          ...
          this.cityOption = GetCityOption(dateData.endyear+'年全省各地市各设备制造商设施数量统计', this.$store.state.cityList, list);
        });
      },
    },
    mounted() {
      // this.init();
      this.$store.state.equipment.subActive = '3';
    },
    computed:{
