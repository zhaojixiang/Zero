var colorScale = {};
colorScale.cmap = new Object;
colorScale.alphaValue = 255;

colorScale.calcColorScale = function(colormapLength) {
	if (colormapLength == 0) {
		colormapLength = 1;
	}
	...
	for (var i = 0; i < colormapLength; i++)
	{
		this.cmap[i, 0] = this.alphaValue;
		for (var j = 0; j < 3; j++)
		{
			this.cmap[i][j + 1] = Math.ceil(cMatrix[i][j] * 255);
		}
	}
}

colorScale.getColor = function(cmin, cmax, val) {
	var colorLength = this.cmap.length;
	var cindex;
	if ((cmax - cmin) <= 0) {
		cindex = 1;
	}
	...
	for (var k = 0; k < 4; k++)
	{
		ARGBArray[k] = this.cmap[cindex - 1][k];
	}
